...
При запуске Maestro показывает короткую цветную инструкцию: что произойдёт, где появится проект и как управлять меню. Затем мастер просит принять не больше трёх решений:

1. выбрать действие: создать новый проект, подключить существующий или проверить его;
2. ввести название нового проекта либо путь к существующему;
3. выбрать исходную точку: идея, материалы, требования или код.

Если при создании ввести только название, например `Мой проект`, Maestro создаст папку на Рабочем столе. Если указать явный путь (`~/Documents/Projects/app`, `/Users/me/Projects/app`, `C:\Projects\app`), он будет использован без добавления Desktop.

Для проверки готового проекта третий вопрос не задаётся: нужны только действие и путь. После выполнения Maestro показывает полный путь и несколько следующих шагов. Цвет можно отключить стандартной переменной `NO_COLOR=1`; текст и маркеры останутся понятными без цвета.

CLI не требует заранее разбираться в manifest, checksums или Git bootstrap.
