...
export function parseMaestroArgs(argv: readonly string[]): ParseResult {
  if (argv.length === 0) {
    return { kind: 'error', message: 'Не указана команда. Доступно: doctor.' };
  }
  ...
  if (first !== 'doctor') {
    return { kind: 'error', message: `Неизвестная команда: ${first}. Доступно: doctor.` };
  }

  const options: DoctorOptions = { path: process.cwd(), json: false };
...
export async function runMaestroCli(argv: readonly string[], io: CliIo = processIo): Promise<number> {
  ...
  if (parsed.kind === 'error') {
    ...
    io.err(`Подсказка: ${SERVICE_COMMAND} --help`);
    return EXIT_USAGE;
  }
